...
task('build', ['clean'], function () {
    ...
    fs.writeFileSync(__dirname + "/pkg/bbui-0.9.4.js", output);
    fs.writeFileSync(__dirname + "/samples/bbui-0.9.4.js", output);

    css += include("src/bbUI.css");
    fs.writeFileSync(__dirname + "/pkg/bbui-0.9.4.css", css);
    fs.writeFileSync(__dirname + "/samples/bbui-0.9.4.css", css);
